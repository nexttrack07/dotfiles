-- function to get default options for map
local get_options = function(opts)
...
M.map = function(mode, target, source, opts)
	vim.api.nvim_set_keymap(mode, target, source, get_options(opts))
end
-- map("i", "jk", "<Esc>")
...
M.buf_map = function(mode, target, source, opts, bufnr)
    vim.api.nvim_buf_set_keymap(bufnr or 0, mode, target, source, get_options(opts))
end
